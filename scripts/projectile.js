...
class Projectile extends Entity {
  ...
  update() {
    ...
    } else if (this.x > this.scene.game.canvas.width + this.width) {
      this.dropParticles();
      explosionSound.play();
      Projectile.deleteProjectile(this);
    }
    if (this.y < -this.height) {
      ...
      explosionSound.play();
      Projectile.deleteProjectile(this);
    } else if (this.y > this.scene.game.canvas.height + this.height) {
      this.dropParticles();
      explosionSound.play();
      Projectile.deleteProjectile(this);
    }
    for (let e of Enemy.enemies) {
      ...
      if (rect.intersects(rect2)) {
        explosionSound.play();
        e.takeDamage(this.scene.player.attack);
        this.dropParticles();
        if (e instanceof Enemy2) {
          if (e.hp <= 0) {
            this.scene.player.xp += e.level;
          }
        } else {
          this.scene.player.xp += 1;
        }
        Projectile.deleteProjectile(this);
      }
    }
