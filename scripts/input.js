...
document.addEventListener("touchmove", (e) => {
  ...
  if (
    inputs.swipeDistanceX < -inputs.treshHold &&
    inputs.swipes.indexOf("swipe right") === -1
  ) {
    inputs.swipes.push("swipe right");
  }
  if (
    inputs.swipeDistanceX > inputs.treshHold &&
    inputs.swipes.indexOf("swipe left") === -1
  ) {
    inputs.swipes.push("swipe left");
  }
  inputs.swipeDistanceY = inputs.touchY - e.changedTouches[0].pageY;
  if (
    inputs.swipeDistanceY < -inputs.treshHold &&
    inputs.swipes.indexOf("swipe down") === -1
  ) {
    inputs.swipes.push("swipe down");
  }
  if (
    inputs.swipeDistanceY > inputs.treshHold &&
    inputs.swipes.indexOf("swipe up") === -1
  ) {
    inputs.swipes.push("swipe up");
  }
});
